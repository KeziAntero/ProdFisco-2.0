...
export default function EditarPRT() {
  ...
  const calcularPontuacao = (servicoId: string, qtdFiscais: number, quantidade?: number) => {
    const servico = servicos?.find(s => s.id === servicoId);
    if (!servico || !qtdFiscais) return 0;
    const qtd = quantidade ?? 1;
    return Number(((servico.pontuacao_base / qtdFiscais) * qtd).toFixed(2));
  };

  const handleServicoChange = (servicoId: string) => {
    setNovoItem(prev => {
      const pontuacao = calcularPontuacao(servicoId, prev.qtd_fiscais, prev.quantidade);
      return {
        ...prev,
  ...
  const handleQtdFiscaisChange = (qtd: number) => {
    setNovoItem(prev => {
      const pontuacao = calcularPontuacao(prev.servico_id, qtd, prev.quantidade);
      return {
        ...prev,
        qtd_fiscais: qtd,
        pontuacao_calculada: pontuacao,
  ...
  const handleQuantidadeChange = (quantidade: number | undefined) => {
    setNovoItem(prev => {
      const pontuacao = calcularPontuacao(prev.servico_id, prev.qtd_fiscais, quantidade);
      return {
        ...prev,
        quantidade,
        pontuacao_calculada: pontuacao,
  ...
  const salvarEdicao = () => {
    ...
    setItens(prev => prev.map(item =>
      item.tempId === editingItem ? { ...novoItem, pontuacao_calculada: pontuacao } : item
    ));
    cancelarEdicao();
  };
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Registro de Produtividade Fiscal - #{registro.numero_registro}
          </p>
        </div>
      </div>
                  ...
                  {itens.map((item) => {
                    ...
                    return (
                      <TableRow key={item.tempId}>
                        ...
                        <TableCell>
  {isEditing 
    ? novoItem.pontuacao_calculada.toFixed(2) 
